...
Many mechanical rules are enforced by ESLint (`eslint-plugin-local/`) — these focus on judgment calls the linter can't catch.
...
- Use UI primitives from `$lib/components/ui/` (Dialog, ConfirmDialog, Input, Select, Tooltip, TabNavigation, CollapsibleInput, Notifications) — never use unstyled `<input>`/`<select>` elements.
