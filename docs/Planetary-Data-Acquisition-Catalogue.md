...
The built-in Solar System preset now uses Mars as the first provider-derived acquisition fixture. Its machine-readable package is served from `/seed-data/celestial/mars/manifest.json`, and `npm run seed:assets:mars` reproducibly fetches and validates the upstream products before rebuilding the runtime plates.
