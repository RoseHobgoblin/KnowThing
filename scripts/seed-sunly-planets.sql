...
INSERT INTO planetary_bodies (
	...
	mass_kg, radius_m, temperature, composition, atmosphere, surface_pressure,
	orbital_period_days, semi_major_axis_au, eccentricity, inclination,
	rotation_period_s, axial_tilt, albedo, satellites, has_rings, epoch_phase,
	description
)
VALUES
	('Cinder', 'cinder', 'planet', (SELECT id FROM stars WHERE slug = 'the-sun'),
	 2.4e23, 2.6e6, '430 °C day / −170 °C night', 'Iron-nickel core, silicate crust', 'None (trace sodium exosphere)', '≈0 bar',
	 89.0, 0.39, 0.21, 6.3, 5.07e6, 0.5, '0.11', 0, false, 0.12,
	...
	('Marrow', 'marrow', 'planet', (SELECT id FROM stars WHERE slug = 'the-sun'),
	 4.6e24, 6.0e6, '462 °C (near-uniform)', 'Basaltic plains, sulfur highlands', 'Dense CO₂ with sulfuric-acid clouds', '88 bar',
	 204.8, 0.68, 0.01, 3.4, 2.10e7, 2.6, '0.75', 0, false, 0.44,
	...
	('Cael', 'cael', 'planet', (SELECT id FROM stars WHERE slug = 'the-sun'),
	 6.4e24, 6.4e6, '14 °C mean', 'Silicate mantle, iron core, liquid-water oceans', 'N₂ / O₂', '1.0 bar',
	 393.0, 1.05, 0.03, 0.0, 8.64e4, 23.4, '0.30', 1, false, 0.78,
	...
	('Rustmere', 'rustmere', 'planet', (SELECT id FROM stars WHERE slug = 'the-sun'),
	 6.9e23, 3.5e6, '−55 °C mean', 'Iron-oxide regolith, polar water ice', 'Thin CO₂', '0.008 bar',
	 753.0, 1.62, 0.09, 1.9, 8.86e4, 25.2, '0.25', 2, false, 0.29,
	...
	('Gorm', 'gorm', 'planet', (SELECT id FROM stars WHERE slug = 'the-sun'),
	 1.6e27, 6.8e7, '−145 °C (cloud tops)', 'Hydrogen / helium over a metallic-hydrogen core', 'H₂ / He with ammonia bands', '— (no solid surface)',
	 4456.0, 5.30, 0.048, 1.3, 3.6e4, 3.1, '0.51', 34, true, 0.61,
	...
	('Halvane', 'halvane', 'planet', (SELECT id FROM stars WHERE slug = 'the-sun'),
	 9.2e25, 2.4e7, '−200 °C', 'Water / ammonia / methane ices over a rocky core', 'H₂ / He / CH₄ (cyan tint)', '— (no solid surface)',
	 11378.0, 9.90, 0.011, 0.8, 6.2e4, 28.3, '0.61', 12, true, 0.05,
	...
	('Vesper', 'vesper', 'planet', (SELECT id FROM stars WHERE slug = 'the-sun'),
	 1.4e22, 1.2e6, '−230 °C', 'Nitrogen-methane ice over rock', 'Tenuous N₂ (seasonal)', '10 µbar',
	 42945.0, 24.0, 0.25, 17.1, 5.5e5, 12.0, '0.50', 1, false, 0.90,
...
INSERT INTO planetary_bodies (
	...
	mass_kg, radius_m, temperature, composition,
	orbital_period_days, semi_major_axis_au, eccentricity, inclination,
	rotation_period_s, axial_tilt, albedo, epoch_phase, description
)
VALUES
	('Mirl', 'mirl', 'planet', (SELECT id FROM stars WHERE slug = 'the-sun'), (SELECT id FROM planetary_bodies WHERE slug = 'cael'),
	 7.3e22, 1.7e6, '−20 °C (sunlit)', 'Anorthosite highland crust',
	 27.3, 0.00257, 0.05, 5.1, 2.36e6, 6.7, '0.12', 0.30,
	...
	('Dask', 'dask', 'planet', (SELECT id FROM stars WHERE slug = 'the-sun'), (SELECT id FROM planetary_bodies WHERE slug = 'rustmere'),
	 1.0e16, 11000, '−60 °C', 'Carbonaceous rock',
	 0.32, 0.00006, 0.015, 1.1, 2.76e4, 0.0, '0.07', 0.60,
	...
	('Pel', 'pel', 'planet', (SELECT id FROM stars WHERE slug = 'the-sun'), (SELECT id FROM planetary_bodies WHERE slug = 'rustmere'),
	 1.5e15, 6200, '−63 °C', 'Dark captured rock',
	 1.26, 0.00016, 0.0003, 0.9, NULL, NULL, '0.07', 0.15,
	...
	('Tamber', 'tamber', 'planet', (SELECT id FROM stars WHERE slug = 'the-sun'), (SELECT id FROM planetary_bodies WHERE slug = 'gorm'),
	 8.9e22, 1.8e6, '−150 °C', 'Sulfur-coated silicate',
	 1.77, 0.0028, 0.004, 0.0, NULL, NULL, '0.60', 0.40,
	...
	('Senn', 'senn', 'planet', (SELECT id FROM stars WHERE slug = 'the-sun'), (SELECT id FROM planetary_bodies WHERE slug = 'gorm'),
	 1.5e23, 2.6e6, '−160 °C', 'Water-ice shell over a subsurface ocean',
	 7.15, 0.0071, 0.009, 0.2, NULL, NULL, '0.68', 0.85,
