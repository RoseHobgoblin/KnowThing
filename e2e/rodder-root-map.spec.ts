...
test.describe('rodder root map', () => {
	test('renders Orrery and Plan at desktop width', async ({ page }) => {
		...
		await expect(canvas).toHaveAttribute('data-texture-lod-total', '15')
		await expect(canvas).toHaveAttribute('data-texture-lod-initial', '256')
		await expect(canvas).toHaveAttribute('data-texture-lod-desired-max', '256')
	...
	test('selects, focuses, follows fractional playback, resets, and resizes', async ({ page }) => {
		...
		await expect(canvas).toHaveAttribute('data-texture-lod-desired-max', '512')
		await expect(canvas).toHaveAttribute('data-texture-lod-pending', '0')
		...
		await canvas.hover()
		for (let step = 0; step < 8; step++) await page.mouse.wheel(0, -300)
		await expect(canvas).toHaveAttribute('data-texture-lod-desired-max', '1024')
		await expect(canvas).toHaveAttribute('data-texture-lod-pending', '0')
