...
<div class="bg-surface shadow-sm">
	...
	{:else}
		...
		<div class="px-6 py-3 border-t border-border-subtle flex justify-between text-sm">
			{#if data.page > 1}
				<a href="/dashboard/recent?page={data.page - 1}" class="text-link hover:text-link-hover">Previous</a>
			{:else}
				<span></span>
			{/if}
			{#if data.edits.length === data.perPage}
				<a href="/dashboard/recent?page={data.page + 1}" class="text-link hover:text-link-hover">Next</a>
			{/if}
		</div>
	{/if}
</div>
