...
<div>
	...
	<form method="POST" use:enhance={() => { submitting = true; return async ({ update }) => { submitting = false; await update() } }} class="flex flex-col h-[calc(100vh-5rem)]">
		...
		<input type="hidden" name="entityKind" value="lexicon" />
		...
		<RecordModeBanner
			modeLabel="Edit Word"
			title="Lexicon Article Editor"
			description="Edit the prose body for this word. Definitions and structured fields belong in the word's contribute screen."
		/>
		...
		<div class="flex items-center justify-between px-6 py-2 bg-surface border-b border-border">
			<h1 class="text-sm font-bold text-secondary truncate">
				...
				{#if data.isMultiple}
					<span class="text-faint text-xs">(homograph)</span>
				{/if}
			</h1>
