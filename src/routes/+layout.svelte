...
<div class="h-screen flex bg-page overflow-hidden" dir={sc?.textDirection ?? 'ltr'}>
	...
	<div class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
		...
		<main class="flex-1 overflow-y-auto flex flex-col">
			<div class="flex-1 {currentPath.includes('/edit') || currentPath === '/know/create' ? 'w-full px-6 py-4' : 'max-w-7xl mx-auto w-full px-4 py-6 md:px-6 md:py-8'}">
				{#key currentPath}
					{@render children()}
				{/key}
			</div>
			<footer class="border-t border-border bg-surface p-4 shrink-0 mt-auto">
				<div class="max-w-7xl mx-auto text-xs text-faint text-center">
					{#if sc?.footerText}
						{sc.footerText}
					{:else}
