<script lang="ts">
	...
	let {
		...
		...rest
	}: Tooltip.RootProps & Pick<Tooltip.ContentProps, 'side' | 'sideOffset'> & {
...
<Tooltip.Root {delayDuration} ignoreNonKeyboardFocus bind:open {...rest}>
	...
	<Tooltip.Portal>
		<Tooltip.Content
			...
			{side}
			class={cn(
				'z-[10000] rounded-md py-2 px-3 text-sm text-body bg-surface border border-border shadow-lg relative',
