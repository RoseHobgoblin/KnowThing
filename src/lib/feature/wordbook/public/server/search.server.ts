...
import { definitions, inflectedForms, languages, lexicon } from '$lib/feature/wordbook/server/schema.server.js'
import { buildWordbookHref } from '$lib/parser/wordbook-path.js'
...
export function listWordbookLanguageOptions() {
	return db.select({ name: languages.name, slug: languages.slug }).from(languages).orderBy(asc(languages.name))
}

export const wordbookSearchProvider: SearchProvider = {
	scope: 'wordbook',
	async search(query) {
		const results = await searchWordbookEntries({
			query: query.q, language: query.filters.language, tag: query.filters.tag,
			pos: query.filters.pos, limit: query.limit, offset: query.offset,
		})
		return results.map(result => ({
			kind: 'word', title: result.word,
			href: buildWordbookHref({ language: result.languageSlug, word: result.word }),
			badge: 'Word', snippet: result.definition ?? '',
			meta: [result.languageName, result.partOfSpeech].filter(Boolean) as string[],
			rank: normalizeRank(result.word, query.q, result.relevance ?? 1),
		}))
	},
	count(query) {
		return countWordbookSearchResults({
			query: query.q, language: query.filters.language, tag: query.filters.tag, pos: query.filters.pos,
		})
	},
}

function normalizeRank(word: string, query: string, relevance: number) {
