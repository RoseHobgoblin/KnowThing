<script lang="ts">
	...
	const classRulesQuery = createQuery(() => ({
		...
		queryFn: () => languageRequest<{ rules?: Array<{ cellKey: string, pattern: string }> }>('GET', languageSlug, `inflections/classes/${selectedClassId}`),
	...
	const saveMutation = createMutation(() => ({
		mutationFn: (body: { classId: number | null, stem: string | null, overrides: Record<string, string> }) =>
			wordbookEntryRequest('PUT', entryId, 'inflection', body),
	}))
