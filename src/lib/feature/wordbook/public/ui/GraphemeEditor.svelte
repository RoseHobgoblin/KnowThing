<script lang="ts">
	...
	const saveMutation = createMutation(() => ({
		mutationFn: ({ id, body }: { id: number | null, body: Record<string, unknown> }) =>
			languageRequest<Grapheme>(id ? 'PATCH' : 'POST', languageSlug, id ? `graphemes/${id}` : 'graphemes', body),
	}))
	const deleteMutation = createMutation(() => ({
		mutationFn: (id: number) => languageRequest('DELETE', languageSlug, `graphemes/${id}`),
	}))
	const restoreMutation = createMutation(() => ({
		mutationFn: (body: Record<string, unknown>) =>
			languageRequest<Grapheme>('POST', languageSlug, 'graphemes', body),
	}))
	const reorderMutation = createMutation(() => ({
		mutationFn: (order: number[]) =>
			languageRequest('POST', languageSlug, 'graphemes/reorder', { order }),
	}))
