<script lang="ts">
	...
	import { searchWordbook, wordbookEntryRequest } from '../wordbook-client.js'
	...
	const deleteMutation = createMutation(() => ({
		mutationFn: (relationId: number) =>
			wordbookEntryRequest('DELETE', entryId, `relations/${relationId}`),
	}))
	...
	const search = createQuery(() => ({
		...
		queryFn: () => searchWordbook<SearchResult[]>(debouncedQuery, 10),
	...
	const addMutation = createMutation(() => ({
		mutationFn: ({ sourceId, targetId, relationType, notes }: {
			...
			relationType: string
			notes?: string
		}) => wordbookEntryRequest('POST', sourceId, 'relations', { targetId, relationType, notes }),
	}))
