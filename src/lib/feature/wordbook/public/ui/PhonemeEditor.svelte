<script lang="ts">
	...
	import { languageRequest } from '../wordbook-client.js'
	...
	const linkedQuery = createQuery(() => ({
		...
		queryFn: () => languageRequest<{ graphemes?: LinkedGrapheme[] }>('GET', languageSlug, `phonemes/${editingId}`),
	...
	const addMutation = createMutation(() => ({
		mutationFn: (body: Record<string, unknown>) =>
			languageRequest<Phoneme>('POST', languageSlug, 'phonemes', body),
	}))
	const saveMutation = createMutation(() => ({
		mutationFn: ({ id, body }: { id: number | null, body: Record<string, unknown> }) =>
			languageRequest<Phoneme>(id ? 'PATCH' : 'POST', languageSlug, id ? `phonemes/${id}` : 'phonemes', body),
	}))
	const deleteMutation = createMutation(() => ({
		mutationFn: (id: number) => languageRequest<{ affectedGraphemes?: number }>('DELETE', languageSlug, `phonemes/${id}`),
	}))
