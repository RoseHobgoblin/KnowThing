...
describe('starlight controller', () => {
	...
	it('uses an explicit ambient presentation when a root has no stars', () => {
		...
		expect(controller.fillLight.intensity).toBe(0)
		expect(controller.summary().label).toContain('unlit in Physical')
		controller.setVisibilityMode('markers')
		expect(controller.fillLight.intensity).toBeGreaterThan(0)
		expect(controller.summary().label).toContain('ambient presentation')
		controller.dispose()
	})
})
