...
import type { RodderMediaPurpose } from './media-binding.js'
...
const bodyConfig: RodderFormConfig = {
		...
		},
		{
			...
			groups: [{
				...
				fields: [
					...
					{ control: 'number', key: 'longitudeAscendingNode', label: 'Ascending Node Ω (deg)', placeholder: '0.0', hint: 'Longitude of the ascending node (Ω), in degrees: swivels the tilted orbital plane around the reference axis. With inclination it fixes the plane in 3D.' },
					{ control: 'number', key: 'argumentOfPeriapsis', label: 'Argument of Periapsis ω (deg)', placeholder: '0.0', hint: 'Argument of periapsis (ω), in degrees: rotates the ellipse within its orbital plane, setting where closest approach points.' },
	...
	computed: [
		...
		{ tab: 'orbit', label: 'Mean orbital speed', compute: ctx => bodyOrbitalDerivations(ctx).meanOrbitalSpeed },
