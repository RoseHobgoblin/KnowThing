...
import { rodderBodies, rodderSectorRoots, rodderSectors } from '$lib/feature/rodder/server/schema.server.js'
...
import { sectorBoundsRadius } from '$lib/feature/rodder/public/sector-view.js'
import type { MapBody } from '$lib/feature/rodder/public/root-layout.js'
import {
	parseRingSystem,
	summarizeRingSystem,
} from '$lib/feature/rodder/public/ring-system.js'
import { resolveRodderModel } from '../../server/models.server.js'
import { getRootMapEntities } from './registry.server.js'
...
export async function resolveRodderEntityDocument(identifier: string, calendarPort?: RodderCalendarPort): Promise<RodderEntityDocument | null> {
	...
	if (isRoot && (entity.kind === 'system' || entity.kind === 'body')) {
		const [map, apparentSky, calendars] = await Promise.all([
			...
			calendarPort?.calendarsForRoot(entity.id) ?? Promise.resolve([]),
		])
...
export async function resolveRodderEntityDocuments(identifiers: readonly string[], calendarPort?: RodderCalendarPort) {
	const unique = [...new Set(identifiers.map(value => value.trim()).filter(Boolean))]
	const entries = await Promise.all(unique.map(async identifier => [identifier, await resolveRodderEntityDocument(identifier, calendarPort)] as const))
