<script lang="ts">
	...
	import RootMapView from '$lib/feature/rodder/public/ui/RootMap.svelte'
	import { resolveRootMapEmbedConfiguration } from '$lib/feature/rodder/public/embed-config.js'
	import {
	...
	} from '$lib/feature/rodder/public/embed-clock.js'
	import { rodderEntityResources } from '../wiki-resources.js'

	let { args }: { args: TemplateArg[] } = $props()

	const ctx = getKnowContext()
	const slug = $derived(positionalArg(args, 0)?.trim() || '')
	const resources = $derived(rodderEntityResources(ctx.extensionResources))
