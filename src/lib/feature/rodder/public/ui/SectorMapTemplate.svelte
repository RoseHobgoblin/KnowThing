<script lang="ts">
	...
	import SectorMapView from '$lib/feature/rodder/public/ui/SectorMap.svelte'
	import { resolveSectorMapEmbedConfiguration } from '$lib/feature/rodder/public/embed-config.js'
	import { rodderViewUrl, type SectorCameraState, type SectorViewState } from '$lib/feature/rodder/public/view-state.js'
	import type { SectorRootView } from '$lib/feature/rodder/public/sector-view.js'
	import { rodderSectorResources } from '../wiki-resources.js'

	let { args }: { args: TemplateArg[] } = $props()

	const ctx = getKnowContext()
	const slug = $derived(positionalArg(args, 0)?.trim() || '')
	const resources = $derived(rodderSectorResources(ctx.extensionResources))
	const document = $derived(slug ? resources?.get(slug) : null)
