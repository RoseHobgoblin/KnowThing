...
import { countries, worldMapRegionGeometry, worldMapRegions, worldMaps } from '$lib/feature/worldmap/server/schema.server.js'
import { contentRecords } from '$lib/server/db/core-schema.js'
...
async function resolveMapSource(imageFilename: string) {
	const mediaRecord = await findMediaRecord(imageFilename)
