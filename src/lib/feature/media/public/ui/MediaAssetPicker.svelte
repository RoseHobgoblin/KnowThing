<script lang="ts">
	...
	import {
		...
		type MediaBindingProfile,
	} from '$lib/feature/media/public/media-binding.js'
	import { getMediaAsset, listMediaAssets, mediaThumbnailUrl, uploadMediaAsset } from '../media-client.js'
	...
	const selectedAssetQuery = createQuery(() => {
		...
		return {
			...
			queryFn: async ({ signal }) => {
				...
				return getMediaAsset(binding, signal)
			},
	...
	const mediaQuery = createInfiniteQuery(() => {
		...
		return {
			queryKey: ['media', 'binding-picker', term, compatibleOnly, pageSize],
			queryFn: ({ pageParam, signal }) => {
				...
				if (profile.compatibility.aspectRatio) params.set('aspectRatio', String(profile.compatibility.aspectRatio))
				return listMediaAssets(params, signal)
			},
			initialPageParam: 0,
	...
	const uploadMutation = createMutation(() => ({
		...
		onSuccess: (item) => {
			...
			const compatibility = assessMediaCompatibility(item, profile.compatibility)
	...
	const selectedCompatibility = $derived(resolved ? assessMediaCompatibility(resolved, profile.compatibility) : null)
	...
	function choose(item: MediaAssetListItem) {
		...
		const binding = mediaBindingFromItem(item, profile.interpretation)
...
<div class="space-y-1.5" data-testid="media-asset-field">
	...
	{#if value}
		<div class="border border-border-subtle bg-page p-2">
			...
			{#if profile.interpretation.normalY !== undefined}
			...
			{:else if profile.interpretation.elevationUnit !== undefined}
				<label class="mt-2 block text-xs text-secondary">
					Elevation values
					<select
						class="mt-1 w-full bg-surface px-2 py-1.5 text-xs text-body outline-none focus:ring-2 focus:ring-accent"
	...
	{:else}
		<button
			...
			onclick={showPicker}
		>
			<ImageSquare size={18} /> Choose {profile.label}
		</button>
	{/if}
</div>

<Dialog bind:open title="Choose {profile.label}" subtitle="Select a compatible image from Media or upload one here." class="max-w-4xl" mainClass="min-h-0">
			...
			{:else}
				<div class="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
					{#each files as file (file.id)}
						{@const compatibility = assessMediaCompatibility(file, profile.compatibility)}
						<button
							type="button"
							class="
						...
						>
							<div class="relative aspect-2/1 overflow-hidden bg-black">
								<img src={mediaThumbnailUrl(file.filename)} alt="" loading="lazy" class="size-full object-contain" />
