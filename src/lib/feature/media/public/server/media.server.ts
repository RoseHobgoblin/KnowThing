...
import {
	...
	mediaHistory,
	mediaVersions,
} from '$lib/feature/media/server/schema.server.js'
import { contentMediaUsage, contentRecords, users } from '$lib/server/db/core-schema.js'
import { getSiteConfig } from '$lib/server/settings.js'
import { sanitizeSvg, stripExifMetadata, verifyMimeType } from '../../server/sanitize.server.js'
...
export async function renameMediaFile(userId: number, oldFilename: string, newFilenameRaw: string) {
	...
	for (const size of THUMB_SIZES) {
		...
		try {
			await fsRename(oldThumb, newThumb)
		} catch {
			// Thumbnail may not exist for legacy uploads.
		}
	}
	...
	if (record.hasRaster) {
		try {
			...
			// A missing derived raster does not invalidate the primary rename.
		}
	}
	...
	for (const v of versions) {
		...
		try {
			await fsRename(v.filepath, newVersionPath)
		} catch {
			// Historical rows can outlive files; preserve the primary rename.
		}
	}
