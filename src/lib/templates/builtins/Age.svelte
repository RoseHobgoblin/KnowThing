<script lang="ts">
	...
	const birthYear = $derived(Number.parseInt(positionalArg(args, 0) || ''))
	const deathYear = $derived(Number.parseInt(positionalArg(args, 1) || ''))

	const endYear: number | undefined = $derived(Number.isNaN(deathYear) ? ctx.calendarDate?.year : deathYear)
	const age = $derived((Number.isFinite(birthYear) && endYear != null) ? endYear - birthYear : null)
</script>
